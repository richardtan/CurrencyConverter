package com.tckr.currencyconverter.view;

import java.util.Collections;
import java.util.ArrayList;

import com.tckr.currencyconverter.R;
...
public class DraggableGridView extends ViewGroup implements View.OnTouchListener, View.OnClickListener, View.OnLongClickListener {
	...
	protected int colCount, childSize, padding, dpi, scroll = 0, childheight = 80;
	...
	public static String LOG_TAG = "com.tckr.currencyconverter.view.DraggableGridView";
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		childheight = childSize / 2;
		for (int i = 0; i < getChildCount(); i++)
			if (i != dragged)
			{
				Point xy = getCoorFromIndex(i);
				getChildAt(i).layout(xy.x, xy.y, xy.x + childSize, xy.y + childSize - childheight);
			}
	}
	@Override
	...
	protected int getRowFromCoor(int coor)
	{
		...
		for (int i = 0; coor > 0; i++)
		{
			if (coor < (childSize - childheight))
				return i;
			coor -= (childSize + padding - childheight);
		}
		return -1;
	}
	...
	protected Point getCoorFromIndex(int index)
	{
		...
		return new Point(padding + (childSize + padding) * col,
						 padding + (childSize + padding - childheight) * row - scroll);
	}
	...
	public boolean onLongClick(View view)
	{
		...
		if (index != -1)
		{
			// Only disable the View Pager scroll if and only if you have the "pager" implemented i.e. you are not a tablet
			try {
				vp = (ViewPager) view.getRootView().findViewById(R.id.pager);
				vp.requestDisallowInterceptTouchEvent(true);
			} catch(Exception e) {
				Log.d(LOG_TAG, "onLongClick() - Cannot disable the pager because it does not exist. You are a Tablet");
			}
		   ...
		   switch (action & MotionEvent.ACTION_MASK) {
			   ...
			   case MotionEvent.ACTION_MOVE:
				   ...
				   if (dragged != -1)
				   {
					   ...
					   int l = x - (3 * childSize / 4), t = y - (3 * (childSize - childheight) / 4);
					   getChildAt(dragged).layout(l, t, l + (childSize * 3 / 2), t + ((childSize - childheight) * 3 / 2));
			   ...
			   case MotionEvent.ACTION_UP:
				   if (dragged != -1)
				   {
					   ...
					   else
					   {
						   ...
						   v.layout(xy.x, xy.y, xy.x + childSize, xy.y + childSize - childheight);
					   }
					   v.clearAnimation();
	...
	{
		View v = getChildAt(dragged);
		int x = getCoorFromIndex(dragged).x + childSize / 2, y = getCoorFromIndex(dragged).y + (childSize - childheight) / 2;
		int l = x - (3 * childSize / 4), t = y - (3 * (childSize - childheight) / 4);
		v.layout(l, t, l + (childSize * 3 / 2), t + ((childSize - childheight) * 3 / 2));
		AnimationSet animSet = new AnimationSet(true);
		ScaleAnimation scale = new ScaleAnimation(.667f, 1, .667f, 1, (childSize - childheight) * 3 / 4, (childSize - childheight) * 3 / 4);
	...
	{
		int rowCount = (int)Math.ceil((double)getChildCount()/colCount), max = rowCount * (childSize- childheight) + (rowCount + 1) * padding - getHeight();
		return max;
	}
